        ...
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-[80%] ${isMe ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-bg-card)] border border-[var(--color-border)]'} rounded-lg p-sm shadow-sm relative group`}>
                
                {/* Reply Context */}
                {msg.replyTo && (
                  <div className={`text-xs mb-xs p-xs rounded ${isMe ? 'bg-white/20' : 'bg-[var(--color-bg)]'} border-l-2 border-white/50`}>
                ...
                {!isMe && <div className="text-xs font-bold mb-xs text-[var(--color-accent)]">{msg.senderName}</div>}
                
                <p className="text-sm">{msg.text}</p>
                
                <span className={`text-[10px] block mt-xs ${isMe ? 'text-white/70' : 'text-muted'}`}>
      ...
      <div className="p-md border-t border-[var(--color-border)] bg-[var(--color-bg-card)]">
        {replyTo && (
          <div className="flex justify-between items-center bg-[var(--color-bg)] p-sm rounded mb-sm border-l-4 border-[var(--color-primary)]">
            <div>
              <span className="text-xs text-[var(--color-primary)] font-bold">Replying to {replyTo.senderName}</span>
              <p className="text-sm text-muted truncate max-w-md">{replyTo.text}</p>
            </div>
            <button onClick={() => setReplyTo(null)} className="text-muted hover:text-red-500">
              <X size={16} />
            </button>
          </div>
        )}
